...
vi.mock("stripe", () => {
  ...
  class StripeCheckout {
    sessions = new StripeCheckoutSessions();
  }
  class StripeMock {
  ...
  const StripeCtor = vi
    .fn()
    .mockImplementation((key: string, opts: any) => new StripeMock(key, opts));
...
describe("checkout-diagnostics", () => {
  ...
  describe("checkStripeKeys()", () => {
    it("fails if secret key missing", async () => {
      ...
      expect(
        diag.results.some(
          (r) => r.step === "Stripe Secret Key" && r.status === "fail"
        )
      ).toBe(true);
    });
    ...
    it("fails if secret key format invalid", async () => {
      ...
      expect(
        diag.results.some(
          (r) => r.step.includes("Secret Key Format") && r.status === "fail"
        )
      ).toBe(true);
    });
    ...
    it("fails if publishable key missing", async () => {
      ...
      expect(
        diag.results.some(
          (r) => r.step === "Stripe Publishable Key" && r.status === "fail"
        )
      ).toBe(true);
    });
    ...
    it("fails if publishable key format invalid", async () => {
      ...
      expect(
        diag.results.some(
          (r) =>
            r.step.includes("Publishable Key Format") && r.status === "fail"
        )
      ).toBe(true);
    });
    ...
    it("fails if key environments mismatch", async () => {
      ...
      expect(
        diag.results.some(
          (r) => r.step === "Stripe Key Pair" && r.status === "fail"
        )
      ).toBe(true);
    });
    ...
    it("passes on valid matching keys", async () => {
      ...
      expect(ok).toBe(true);
      expect(
        diag.results.find((r) => r.step === "Stripe Secret Key")?.status
      ).toBe("pass");
      expect(
        diag.results.find((r) => r.step === "Stripe Publishable Key")?.status
      ).toBe("pass");
    });
  });
  ...
  describe("testStripeConnection()", () => {
    it("fails when no API key", async () => {
      ...
      expect(
        diag.results.some(
          (r) => r.step === "Stripe Connection" && r.status === "fail"
        )
      ).toBe(true);
    });
    ...
    it("passes and warns based on account flags", async () => {
      ...
      mock.account.retrieve = vi.fn(async () => ({
        id: "acct_1",
        email: "acct@example.com",
      ...
      expect(ok).toBe(true);
      expect(
        diag.results.find((r) => r.step === "Stripe Connection")?.status
      ).toBe("pass");
      expect(
        diag.results.find((r) => r.step === "Account Status")?.status
      ).toBe("warning");
    });
  });
  ...
  describe("testCheckoutSessionCreation()", () => {
    it("fails when no API key", async () => {
      ...
      expect(
        diag.results.some(
          (r) => r.step === "Session Creation" && r.status === "fail"
        )
      ).toBe(true);
    });
    ...
    it("passes when session created and retrievable", async () => {
      ...
      expect(ok).toBe(true);
      expect(
        diag.results.find((r) => r.step === "Session Creation")?.status
      ).toBe("pass");
      expect(
        diag.results.find((r) => r.step === "Session Retrieval")?.status
      ).toBe("pass");
    });
  });
  ...
  describe("testAPIRoute()", () => {
    it("fails on non-ok response", async () => {
      (global as any).fetch = vi.fn(
        async () => ({ ok: false, status: 500, text: async () => "err" }) as any
      );
      const ok = await diag.testAPIRoute();
      expect(ok).toBe(false);
      expect(diag.results.find((r) => r.step === "API Route")?.status).toBe(
        "fail"
      );
    });
    ...
    it("fails on missing fields", async () => {
      ...
      expect(ok).toBe(false);
      expect(
        diag.results.find((r) => r.step === "API Route Response")?.status
      ).toBe("fail");
    });
  ...
  describe("checkClientSideCode()", () => {
    it("fails when checkout page missing", () => {
      ...
      expect(ok).toBe(false);
      expect(diag.results.find((r) => r.step === "Checkout Page")?.status).toBe(
        "fail"
      );
    });
    ...
    it("passes page existence and warns about component if missing", () => {
      ...
      expect(
        diag.results.find((r) => r.step === "Checkout Component")?.status
      ).toBe("warning");
    });
    ...
    it("passes and checks component heuristics", () => {
      ...
      expect(ok).toBe(true);
      expect(
        diag.results.find((r) => r.step === "Stripe Key Usage")?.status
      ).toBe("pass");
      expect(
        diag.results.find((r) => r.step === "Redirect Logic")?.status
      ).toBe("pass");
      expect(
        diag.results.find((r) => r.step === "Error Handling")?.status
      ).toBe("pass");
      expect(
        diag.results.find((r) => r.step === "Loading States")?.status
      ).toBe("pass");
    });
  });
  ...
  describe("checkSuccessPage()", () => {
    it("fails when missing", () => {
      ...
      expect(ok).toBe(false);
      expect(diag.results.find((r) => r.step === "Success Page")?.status).toBe(
        "fail"
      );
    });

    it("passes and warns if session id not referenced", () => {
      fsState[repo("app/checkout/success/page.tsx")] =
      ...
      expect(
        diag.results.find((r) => r.step.includes("Session ID"))?.status
      ).toBe("warning");
    });

    it("passes and session id check passes", () => {
      fsState[repo("app/checkout/success/page.tsx")] =
        'const session_id = "abc"; export default function P(){ return null }';
      const ok = diag.checkSuccessPage();
      expect(ok).toBe(true);
      expect(
        diag.results.find((r) => r.step.includes("Session ID"))?.status
  ...
  describe("checkCartStore()", () => {
    it("warns when no store found", async () => {
      ...
      expect(ok).toBe(false);
      expect(diag.results.find((r) => r.step === "Cart Store")?.status).toBe(
        "warning"
      );
    });
    ...
    it("passes when zustand store with items array", async () => {
      ...
      expect(
        diag.results.find((r) => r.step === "Cart Store Implementation")?.status
      ).toBe("pass");
      expect(diag.results.find((r) => r.step === "Cart Items")?.status).toBe(
        "pass"
      );
    });
  });
  ...
  describe("runAllDiagnostics()", () => {
    ...
    it("runs full suite on valid env", async () => {
      ...
      fsState[repo("app/checkout/success/page.tsx")] =
        'const session_id = "x";';
      fsState[repo("components/shop/checkout-page.tsx")] =
        'const k = process.env.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY; try{}catch(e){} const loading = true; window.location.href = "";';
      fsState[repo("lib/stores/cart.ts")] =
        'import { create } from "zustand"; export const s = create({ items: [] });';
    ...
    it("documents that non-development checkout should fail fast when products missing", async () => {
      // This is a documentation-only check to ensure awareness; actual behavior is covered in API tests
      // We assert that NODE_ENV !== 'development' is expected in CI
