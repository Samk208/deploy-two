...
test.describe('Supplier Commission Management', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.route('**/api/products**', async (route) => {
      ...
      if (route.request().method() === 'GET' && !url.pathname.includes('[id]')) {
        // List products
        await route.fulfill({
          status: 200,
          contentType: 'application/json',
  ...
  test('supplier can create product with commission percentage', async ({ page }) => {
    await page.goto('/dashboard/supplier/products/new')

    // Fill product form
    await page.fill('[data-testid="product-title"]', 'New Commission Product')
    await page.fill('[data-testid="product-description"]', 'A product with custom commission')
    ...
    await page.fill('[data-testid="product-images"]', 'https://example.com/product.jpg')
  ...
  test('supplier can edit commission percentage on existing product', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="product-commission"]')).toBeVisible()

    // Verify current commission value
    await expect(page.locator('[data-testid="product-commission"]')).toHaveValue('15')
  ...
  test('commission validation enforces 0-95% range', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="commission-error"]')).toBeVisible()
    await expect(page.locator('[data-testid="commission-error"]')).toContainText('Commission must be between 0-95%')
    ...
    await page.fill('[data-testid="product-commission"]', '25')
    await page.selectOption('[data-testid="product-category"]', 'Electronics')
    await page.fill('[data-testid="product-images"]', 'https://example.com/image.jpg')
    await page.check('[data-testid="region-kr"]')
    
    await page.click('[data-testid="submit-product"]')
    
    // Should not show commission error
    await expect(page.locator('[data-testid="commission-error"]')).not.toBeVisible()
  })
  ...
  test('supplier dashboard displays commission metrics', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="commission-earned"]')).toContainText('$2,490')

    // Verify total revenue KPI
    await expect(page.locator('[data-testid="total-revenue"]')).toBeVisible()
    await expect(page.locator('[data-testid="total-revenue"]')).toContainText('$12,450')

    // Verify today's performance section
    await expect(page.locator('[data-testid="today-revenue"]')).toContainText('$450')
    await expect(page.locator('[data-testid="today-sales"]')).toContainText('3')
    await expect(page.locator('[data-testid="today-orders"]')).toContainText('2')
  })
  ...
  test('top products show commission percentages', async ({ page }) => {
    ...
    await expect(firstProduct.locator('[data-testid="commission-badge"]')).toContainText('15%')
  ...
  test('supplier can only edit own products', async ({ page }) => {
    ...
    await page.route('**/api/products/other-supplier-product', async (route) => {
    ...
    await expect(page.locator('[data-testid="error-message"]')).toContainText('You can only edit your own products')
  })
  ...
  test('products can be filtered by region restrictions', async ({ page }) => {
    ...
    // Apply region filter
    await page.selectOption('[data-testid="region-filter"]', 'KR')
    await page.click('[data-testid="apply-filters"]')

    // Verify filtered results
    await expect(page.locator('[data-testid="product-kr-product"]')).toBeVisible()
    await expect(page.locator('[data-testid="product-kr-product"]')).toContainText('Korea Only Product')
  })

  test('commission earnings are calculated correctly in orders', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="order-total"]')).toContainText('$200.00')
    await expect(page.locator('[data-testid="commission-earned"]')).toContainText('$30.00')
    await expect(page.locator('[data-testid="commission-rate"]')).toContainText('15%')
  })
})
...
test.describe('Supplier Commission Edge Cases', () => {
  test('handles zero commission products', async ({ page }) => {
    ...
    await page.fill('[data-testid="product-description"]', 'No commission product')
  ...
  test('handles maximum commission of 95%', async ({ page }) => {
    ...
    await page.fill('[data-testid="product-description"]', 'Maximum commission product')
    await page.fill('[data-testid="product-price"]', '100')
