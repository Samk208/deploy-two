...
test.describe('Enhanced Shop Features', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="product-card"], .text-center', { timeout: 10000 })
  })
  ...
  test('should filter products by category', async ({ page }) => {
    ...
    const categoryButton = page.locator('button:has-text("Electronics")').first()
  ...
  test('should show quick view modal', async ({ page }) => {
    ...
    const productCard = page.locator('[data-testid="product-card"], .group').first()
  ...
  test('should toggle wishlist', async ({ page }) => {
    ...
    const wishlistButton = page.locator('button:has([data-testid="heart"])').first()
  ...
  test('should be mobile responsive', async ({ page }) => {
    ...
    await expect(page.locator('button:has-text("Filters & Sort")')).toBeVisible()
