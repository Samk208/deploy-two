...
async function checkProductImages() {
  ...
  if (error) {
    addResult('Product Images Query', 'fail', `Database error: ${error.message}`)
    return
  }
  ...
  addResult('Product Images Query', 'pass', `Found ${products.length} products`)
  ...
  for (const product of products as any[]) {
    ...
    if (!Array.isArray(images)) {
      malformedArrays++
      console.log(`⚠️  Product "${title}": images is not an array (type: ${typeof images})`)
      continue
    }
    ...
    if (firstImage.startsWith('"') || firstImage.startsWith("'")) {
      quotedStrings++
      console.log(`⚠️  Product "${title}": image has extra quotes: ${firstImage}`)
    }
  ...
  if (nullImages > (products.length * 0.3)) {
    addResult(
      ...
      ['Consider adding placeholder images', 'Or seed with actual product images']
    )
  }
...
async function checkInfluencerShops() {
  ...
  if (error) {
    addResult('Influencer Shops Query', 'fail', `Database error: ${error.message}`)
    return
  }
  ...
  const influencerIds = Array.from(new Set((shops as any[])
    .map(s => (s as any).influencer_id)
  ...
  if (influencerIds.length > 0) {
    ...
    let fetched = false
    {
      ...
      if (!profErr && Array.isArray(profiles)) {
        avatarById = new Map<string, string | null>(profiles.map(p => [String((p as any).id), (p as any).avatar_url || null]))
        fetched = true
      }
    }
    if (!fetched) {
      ...
      if (!usersErr && Array.isArray(users)) {
        avatarById = new Map<string, string | null>(users.map(u => [String((u as any).id), (u as any).avatar || null]))
        fetched = true
      }
    }
    if (fetched) {
...
function checkImageComponents() {
  ...
  addResult('Image Components', 'pass', 'Manual verification needed', recommendations)
}
